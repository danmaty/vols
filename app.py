...
c1 = st.columns(1)
c1.header(f"Volumes at {data.iloc[1]['datetime'][6:]}")
c1.metric(label='Returns Arrived', value=ar[1], delta=ar[1]-ar[0])
c1.metric(label='Returns Scheduled', value=sr[1], delta=sr[1]-sr[0])
c1.metric(label='A-Stock Arrived', value=aa[1], delta=aa[1]-aa[0])
c1.metric(label='A-Stock Scheduled', value=sa[1], delta=sa[1]-sa[0])
